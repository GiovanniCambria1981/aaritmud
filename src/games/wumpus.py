...
class Wumpus(object):
    ...
    def get_related_attr_name(self, obj):
        if not obj:
            log.bug("obj non � un parametro valido: %r" % obj)
            return ""
...
def import_all_wumpus_gamescripts():
    ...
    for area in database["areas"].values():
        if not area.wumpus:
            continue
        # Per tutto il contenuto dell'area wumpus controlla se una delle entit�
        # � uguale come codice ad una indicata nella istanza della classe Wumpus,
        # se � cos� allora importa nel dato relativo al codice tutte le funzioni
...
def get_wumpus_gamescripts_module(obj, attr_name=""):
    """
    Importa tutti i necessari gamescript relativi ad un'entit� di un'area wumpus.
    """
    if not obj:
        log.bug("obj non � un parametro valido: %r" % obj)
        return None
    ...
    # Se non � stato passato il nome dell'attributo della classe Wumpus a cui
    # appartiene l'obj passato allora � normale che obj non sia uno di quelli
    if not attr_name:
    ...
    if not os.path.exists(filepath):
        log.bug("Non � stato trovato nessun file %s richiesto per i gamescripts di %s" % (filepath, obj.code))
        return None
