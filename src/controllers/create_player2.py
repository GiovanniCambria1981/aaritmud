...
class CreatePlayer2Page(WebResource):
    ...
    def render_GET(self, request, conn):
        ...
        #    return "\nImpossibile creare il personaggio %s perch gi esistente.\n" % conn.player.name
    ...
    def render_POST(self, request, conn):
        if not conn.new_player:
            return '''Per qualche errore sul server il personaggio non  stato creato, oppure  stato gi creato, controlla la pagina con <a href="players.html">i tuoi personaggi</a>.'''
        ...
        try:
            ...
            err_msg_age = "L'et deve essere un numero che indica gli anni vissuti dal tuo personaggio"
        else:
            if age < conn.new_player.race.age_adolescence or age > conn.new_player.race.age_old:
                err_msg_age = "I limiti dell'et, relativamente alla razza da te scelta, sono tra %d e %d." % (
        ...
        err_msg_hand = ""
        if not hand:
            err_msg_hand = "Scegli qual' la %s principale del tuo futuro personaggio" % conn.new_player.race.hand
            hand = HAND.NONE
        ...
        and not err_msg_eyecolor and not err_msg_skin_color and not err_msg_hand and not err_msg_name):
            ...
            # Se c' solo il nuovo personaggio nel database allora gli dona
